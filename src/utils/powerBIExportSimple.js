 ...
 */
const generatePowerQueryM = (projectName, headers, dataTypes) => {
  const typeTransforms = headers.map(h => {
    const type = dataTypes[h];
    let mqType = 'type text';

    if (type === 'number') mqType = 'type number';
    else if (type === 'date') mqType = 'type date';
    else if (type === 'boolean') mqType = 'type logical';

    return `    {"${h}", ${mqType}}`;
  }).join(',\n');

  return `// Power Query M Code - Copy this into Power BI Desktop
// Go to: Home > Transform data > Advanced Editor

let
    // STEP 1: Update this path to where you saved the CSV file
    Source = Csv.Document(
        File.Contents("C:\\\\Path\\\\To\\\\${projectName}_data.csv"),
